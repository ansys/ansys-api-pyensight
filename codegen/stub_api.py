...
class ProcessAPI:
    ...
    def _process_class(self, node: ElementTree.Element, indent: str = "") -> str:
        ...
        if superclass != "ENSOBJ":
            s += f"from ansys.api.pyensight.{superclass.lower()} import {superclass}\n"
        ...
        with open(pathname, "w", encoding="utf-8") as fp:
            fp.write(s)
        return f"from ansys.api.pyensight.{classname.lower()} import {classname}\n"
    ...
    def process(self, dirname: str, filename: str) -> None:
        ...
        assets_file_name = os.path.join(root, "src", "ansys", "api", "pyensight","ensight_api_test_assets.txt")
...
def generate_stub_api() -> None:
    ...
    os.chdir(root)
    target_dir = "../src/ansys/api/pyensight"
    # Get the default Ansys version number
    with open(os.path.join(target_dir, "VERSION"), "r", encoding="utf8") as versionfile:
        VERSION = versionfile.read()
        print(f"Generating API for release {VERSION}")

    # get the API file(s)
    api_name = os.path.join(target_dir, "v0", "ensight_api.xml")
